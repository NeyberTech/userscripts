...
function getTitleThEl(parentEl = document){
    return [].find.call(parentEl.getElementsByClassName('styx-table-column-has-action'), (el)=>['标题', '标题\n⭐️'].includes(el.innerText));
}
...
(async function() {
    ...
    function handleTitleCopyBtnClick(e){
        ...
        alert(`已复制当前页面所有卡片的标题${downingKey.Alt ? '（仅包含带⭐️项目）':''}`);
    }

    function handleStatusCopyBtnClick(e){
        e.stopPropagation();
        copy(getItemStatusAndNameListText());
        alert(`已复制当前页面所有卡片的 状态+标题${downingKey.Alt ? '（仅包含带⭐️项目）':''}`);
    }
    ...
    function createElementOfCopyButton(){
        ...
        titleCopyBtn._AltDowningStyle = downingKey.Alt;
        titleCopyBtn.innerHTML = `
            ${downingKey.Alt ? '<span style="position: absolute; z-index: 1; font-size: 12px; right: 2px; bottom: 2px;">⭐️</span>':''}
    ...
    function refresh(parentEl = document){
        ...
        function refreshCopyButton(parentEl, onClickFn){
            ...
            if (copyBtn && copyBtn._AltDowningStyle != downingKey.Alt) {
