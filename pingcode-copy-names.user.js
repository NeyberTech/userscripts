...
function getStatusThEl(parentEl = document){
    return [].find.call(parentEl.getElementsByClassName('styx-table-column-has-action'), (el)=>el.innerText=='状态');
}
...
(async function() {
    ...
    function getItemNameListText(){
        return getItemNameList().join('\n');
    }

    function getItemStatusAndNameListText(){
        const names = getItemNameList();
        const status = getItemStatusList();
        return status.map((statusText, i)=>{
    ...
    function refresh(parentEl = document){
        ...
        if (titleTh) {
            let titleCopyBtn = [].find.call(titleTh.childNodes, (n)=>n._copyBtn);
            if(!titleCopyBtn) {
                titleCopyBtn = createElementOfCopyButton();
                titleCopyBtn._copyBtn = true;
        ...
        if (statusTh) {
            let statusCopyBtn = [].find.call(statusTh.childNodes, (n)=>n._copyBtn);
            if(!statusCopyBtn) {
                statusCopyBtn = createElementOfCopyButton();
                statusCopyBtn._copyBtn = true;
                statusCopyBtn.onclick = handleStatusCopyBtnClick;
                statusTh.appendChild(statusCopyBtn);
            }
        }
    }
